...
gulp.task('zip', function () {
 return gulp.src([
   '*',
   './fonts/*', //includes these folders
   './inc/*',
   './js/**/*',
   './languages/*',
   './sass/**/*',
   './template-parts/*',
   '!bower_components', //ignore bower components when zipping
   '!node_modules', //igmore node modules when zipping
  ], {base: "."})
  .pipe(zip('strappedin.zip')) //outputs this zip
  .pipe(gulp.dest('.'));
});
